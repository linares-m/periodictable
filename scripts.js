...
async function elCreate(){
  ...
  elW = 60, elH = elW, gap = elW/25, groups = 18, periods = 7, tbW = ((elW+gap)*(groups+2)), tbH = ((elH+gap)*(periods+5));
  //Element Data Points
  var group = (d) => d.xpos, period = (d) => d.period, category = (d) => d.category, name = (d) => d.name, symbol = (d) => d.symbol,
  atomicMass = (d) => d.atomic_mass, atomicMassRound = (d) => d.atomic_mass.toFixed(0), cpk = (d) => d.cpk, summary = (d) => d.summary;
  number = (d) => d.number, phase = (d) => d.phase, source = (d) => d.source, spectralImg = (d) => d.spectral_img,
  ...
  .attr("role", "img")
  .attr("title", name)
  .attr("data-name", name).attr("data-number", number).attr("data-group",	group).attr("data-period", period)
  .attr("data-summary", summary).attr("data-discovered-by", discoveredBy).attr("data-source", source)
  .attr("data-phase", phase).attr("data-color", color).attr("data-appearance", appearance).attr("data-spectral-img", spectralImg)
  .attr("data-atomic-mass", atomicMass).attr("data-density", density).attr("data-melting-point", meltingPoint).attr("data-boiling-point", boilingPoint).attr("data-molar-heat", molarHeat)
  .attr("data-electron-config", electronConfig).attr("data-electron-affin", electronAffin).attr("data-shells", shells).attr("data-electroneg_pauling", electronegPauling)
  .attr("data-class", "g")
  .attr("role", "button")
  .attr("aria-label", "Click For More Info About This Element")
  ...
  .attr("class", symbol)
  .attr("data-category", category)
  .attr("x", (d) => d.xpos *(elW + gap)).attr("y", ((d) => d.ypos * (elH + gap)))
  .attr("height", elH).attr("width", elW)
  .attr("fill", cpk)
  .attr("title", name)
  ...
  .attr("aria-hidden", "true")
  .attr("class", symbol)
  .attr("data-class", "number")
  .attr("x", ((d) => d.xpos * (elW + gap - (number.length/2))+(elW/5)))
  .attr("y", ((d) => d.ypos * (elH + gap)+(elH/4)))
  .attr("width", elW)
  .attr("fill", "black")
  .style("font-family", "'arbutus slab'")
  ...
  .attr("aria-hidden", "true")
  .attr("class", symbol)
  .attr("data-class", "atomicMassLabel")
  .attr("x", ((d) => d.xpos * (elW + gap)+(elW/8)))
  .attr("y", ((d) => d.ypos * (elH + gap)+(elH) -5 ))
  .attr("width", elW)
  .attr("fill", "black")
  .style("font-family", "'arbutus slab'")
  .style("font-size", elW/4);
  ...
  for (i=0; i < elements.length; i++){
    ...
    function selectElement(){
      ...
      || (document.querySelector(".iRI p").innerHTML == "Click on another element to see more about it")) {
        ...
        document.querySelector("#qD2").innerHTML =   "Density: " + datum.density + " (g/l for gases, g/cm^3 for solids/liquids)"
        document.querySelector("#qD3").innerHTML = "Boiling Point: " + datum.boilingPoint + "K"
        document.querySelector("#qD4").innerHTML ="Melting Point: " + datum.meltingPoint + "K"
